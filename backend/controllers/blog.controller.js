...
export const getBlogs = async (req, res) => {
  try {
    ...
    const blogsWithFullImagePath = blogs.map((blog) => ({
      ...blog.toObject(),
      image: `https://bamosbe-m9on.onrender.com/assets/${blog.image}`,
    }));
...
export const createBlog = async (req, res) => {
  try {
    ...
    const blogWithImage = {
      ...newBlog.toObject(),
      image: `https://bamosbe-m9on.onrender.com/assets/${newBlog.image}`,
    };
    ...
    res.status(200).json({
      ...
      data: {
        ...
        image: updatedBlog.image
          ? `https://bamosbe-m9on.onrender.com/assets/${updatedBlog.image}`
          : null,
      },
    });
  } catch (error) {
